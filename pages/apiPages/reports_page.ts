...
export class ApiReportsPage {
    ...
    async reportUser(url: string, userToken: string, reason: 'streamerUnder17' | 'nudity' | 'sexualContent' | 'displayOfCriminalActivitiesOrWeapon' | 'drugsAlcoholSmoking' | 'harassmentHatefulSpeechBullying' | 'sharingPrivateInformation' | 'spam' | 'other', userId: string) {
    ...
    async adminReport(url: string, adminToken: string, reason: 'streamerUnder17' | 'nudity' | 'sexualContent' | 'displayOfCriminalActivitiesOrWeapon' | 'drugsAlcoholSmoking' | 'harassmentHatefulSpeechBullying' | 'sharingPrivateInformation' | 'spam' | 'other', humanId: string) {
        const apiContext = await request.newContext({ignoreHTTPSErrors: true})
