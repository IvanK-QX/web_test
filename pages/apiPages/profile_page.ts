...
export class ApiProfilePage {
    ...
    async inviteToSteram(url: string, userToken: string, option: boolean) {
        ...
        const apiRequest = await apiContext.post(`${url}:3000/profile/setAllowedInviteToStream`, {
            data,
            headers: headers,
        });
        expect(apiRequest.ok()).toBeTruthy();
    ...
    async allowedToStartPremium(url: string, adminToken: string, userId: string, option: boolean) {
        ...
        const apiRequest = await apiContext.post(`${url}:3000/admin/profile/allowedToStartPremium`, {
            data,
            headers: headers,
        });
        expect(apiRequest.ok()).toBeTruthy();
