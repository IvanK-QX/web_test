...
export class ApiModerationsPage {
    ...
    async forbiddenWordDelete(url: string, adminToken: string, forbiddenWordId: string) {
        ...
        const apiRequest = await apiContext.post(`${url}:3000/moderation/forbiidenWords/delete`, {
            data,
            headers: headers,
        });
        expect(apiRequest.ok()).toBeTruthy();
    ...
    async abusiveWordDelete(url: string, adminToken: string, abusiveWordId: string) {
        ...
        const apiRequest = await apiContext.post(`${url}:3000/moderation/abusiveWords/delete`, {
            data,
            headers: headers,
        });
        expect(apiRequest.ok()).toBeTruthy();
    ...
    async actionsOnOtherUser(url: string, userToken: string, userId: string) {
        ...
        const apiRequest = await apiContext.post(`${url}:3000/moderation/actionsOnOtherUser`, {
            data,
            headers: headers,
        });
        expect(apiRequest.ok()).toBeTruthy();
    ...
    async suspendActionRemove(url: string, adminToken: string, actionId: string) {
        ...
        const apiRequest = await apiContext.post(`${url}:3000/moderation/suspend-action/remove`, {
            data,
            headers: headers,
        });
        expect(apiRequest.ok()).toBeTruthy();
    ...
    async getUpdatedUsersList(
        url: string,
        adminToken: string,
        moderatorHumanReadableId: number,
